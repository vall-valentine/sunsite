...
def register():
    ...
    if form_reg.validate_on_submit():
        ...
        post('http://sunsite.herokuapp.com/api/users', json={
...
def open_post(post_id):
    ...
    if request.method == "POST":
        post('http://sunsite.herokuapp.com/api/comments', json={
            'post_id': post_id,
...
def create_post():
    ...
    if form_cr.validate_on_submit():
        post('http://sunsite.herokuapp.com/api/posts', json={
...
def edit_post(post_id):
    ...
    if request.method == "GET":
        posts = get(f'http://sunsite.herokuapp.com/api/posts/'
                    f'{post_id}').json()['post']
    ...
    if request.method == "POST":
        db_session.global_init("db/database.sqlite")
        put(f'http://sunsite.herokuapp.com/api/posts/{post_id}', json={
            'title': form_cr.title.data,
            'content': form_cr.content.data,
            'author': current_user.id
        })
...
def delete_post(post_id):
    """Удаление поста"""
    delete(f'http://sunsite.herokuapp.com/api/posts/{post_id}')
    return redirect('/')
...
def delete_comm(comm_id):
    ...
    cur_post = get(f'http://sunsite.herokuapp.com/api/comments/'
                   f'{comm_id}').json()['comment']['post_id']
    delete(f'http://sunsite.herokuapp.com/api/comments/{comm_id}')
...
def user_page(nickname):
    ...
    if request.method == "POST":
        ...
        put(f'http://sunsite.herokuapp.com/api/users/{current_user.id}', json=body)
...
def chats():
    ...
    chats_for_page = get(f'http://sunsite.herokuapp.com/api/users/'
...
def chat(chat_id):
    ...
    if request.method == "POST":
        ...
        post('http://sunsite.herokuapp.com/api/messages', json={
            'chat': chat_id,
...
def create_chat_function():
    ...
    if users:
        # создание чата
        post('http://sunsite.herokuapp.com/api/chats', json={
...
def edit_chat(chat_id):
    ...
    if request.method == "POST":
        ...
        put(f'http://sunsite.herokuapp.com/api/chats/{chat_id}', json={
            'title': title
        })
        return redirect('/chats')
    return render_template('edit_chat.html',
...
def delete_chat(chat_id):
    """Удаление чата"""
    delete(f'http://sunsite.herokuapp.com/api/chats/{chat_id}')
...
def delete_mess(mess_id):
    """Удаление сообщения"""
    cur_chat = get(f'http://sunsite.herokuapp.com/api/messages/'
                   f'{mess_id}').json()['message']['chat']
    delete(f'http://sunsite.herokuapp.com/api/messages/{mess_id}')
